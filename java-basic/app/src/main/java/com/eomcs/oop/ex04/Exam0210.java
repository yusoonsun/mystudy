...
public class Exam0210 {
  public static void main(String[] args) throws Exception {
    ...
    // s1 과 비교 하였을때, 다른 문자열 'o'와 'i'가 왼쪽으로 6자리 차이남 (+6)
    System.out.println(s1.compareTo("Hello"));
    // s1 과 비교 하였을때, 같으므로 '0'이 출력됨
    System.out.println(s1.compareTo("Hellq"));
    // s1 과 비교 하였을때, 다른 문자열 'o'와 'q'가 오른쪽으로 2자리 차이남 (-2)

    System.out.println(s1.contains("ll"));
    // s1 에 'll' 이 포함되므로 true
    System.out.println(s1.contains("ee"));
    // s1 에 'ee' 이 포함되지 않으므로 false
    ...
    // s1 레퍼런스가 가리키는 인스턴스 주소를 "Hello" 와 같다고 하였으므로 false
